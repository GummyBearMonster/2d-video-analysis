classdef frame_fast
    ...
    methods
        function this_frame=frame_fast(s,image,frame_i)
            if nargin>0
                this_frame.id_cnt=0;
        ...
        function this_frame=writeFrame(this_frame,vo,bw_im)
            I = double(imresize(bw_im,1,'bilinear'));
            
            for i=1:size(this_frame.bubbles,2)
                temp_bubble=this_frame.bubbles{1,i};
                
                if temp_bubble.coalesced
                    I = insertShape(I,'FilledCircle',[temp_bubble.centroid*1 4],'Color','red','Opacity',1);
                else
                    I = insertShape(I,'FilledCircle',[temp_bubble.centroid*1 4],'Color','blue','Opacity',1);
                end
                I = insertText(I,temp_bubble.centroid*1,temp_bubble.id,'BoxColor','white','FontSize',16,'Anchorpoint','LeftCenter');
            end
