v=VideoReader('20psi-0.5lpm-500fps.mp4');
fps=v.FrameRate;
...
for h=1000%:nframes
    g=rgb2gray(read(v,h));
%      g1=g-rgb2gray(bg);
%     g2=rgb2gray(bg)-rgb2gray(read(v,h));
%     
    g=g(ymin-1:ymax+1,xmin-1:xmax+1);
%     g1=g1(ymin-1:ymax+1,xmin-1:xmax+1);
%     g2=g2(ymin-1:ymax+1,xmin-1:xmax+1);% +1 is needed here to enable search
%     g3=g+g2;
     g1=imsharpen(g,'Radius',1.5,'Amount',5);
     I = imbinarize(g,0.5); % Global Otsu to identify white bubble areas
%     I=bwareaopen(I,50);
...
%     I=imopen(I,st);
%     I = uint8(255 * I);
     %g3=imgaussfilt(g,15,'FilterSize',7,'FilterDomain','spatial');
     g3=imgaussfilt(g,5);%,'FilterSize',7,'FilterDomain','spatial');
%      I=imbinarize(I);
     
    o = testAdaptiveThreshold;
    I2=javaMethod('adaptiveBinarization',o,g1,9,0); 
    I2=logical(I2);  % Adaptive threshold to define boundaries
    I3=javaMethod('adaptiveBinarization',o,g3,49,1); 
    I3=logical(I3);
    I3=imdilate(I3,st2);
    I3=imclose(I3,st);
    %I3=imcomplement(I3);
...
%         
%     end
%     I4=I1; %I4 is the combined result
%     y=m+1;
%     input=I2;
%     I3=I1; %I3 is a copy of I1 for each cycle
%     n=0;
%     
...
%     end
%     
% %    erode the bottom 20 pixels
% %     w=1;
% %     
% %     while w>0   % Erode all boundaries to 1 pixel
% %         w=0;
% %         
% %         for i=size(input,1)-20:size(input,1)-1 % Search through all black blocks
% %             for j=2:size(input,2)-1
% %                 if I3(i,j)==0 && (~isEdge(input,i,j) && ~isEdge(I1,i,j)) && (I3(i-1,j)==1 || I3(i,j-1)==1||I3(i,j+1)==1) 
% %                     %if the black block is not an edge block (see isEdge)
% %                     %and if it neigbours a white block, change it to white
% %                     input(i,j)=1;
% %                     I1(i,j)=1;
...
% %     end
%     
% %   reserve 1 pixel at the boundaries (so it's 2 or 3 pixels)
%     for i=3:size(I1,1)-2
%         for j=3:size(I1,2)-2
%             if I2(i,j)==0 && I3(i,j)==1 && (isEdge(I3,i+1,j)||isEdge(I3,i-1,j)||isEdge(I3,i,j+1)||isEdge(I3,i,j-1) )||isEdge(input,i+1,j)||isEdge(I3,i-1,j)||isEdge(I3,i,j+1)||isEdge(I3,i,j-1)
%                 I1(i,j)=0;
%             end
%             if I1(i,j)==0 && I4(i,j)==1
%                 I1(i,j)=1;
%             end
%         end
%     end
...
%    I4=I4(2:size(g,1)-1,2:size(g,2)-1);
...
%        figure
     imshowpair(I3,g,'montage')
%         imshow(g)
%         binarizedArray(:,:,h) = I3;
%     binarizedArray2(:,:,h) = I1;
     %imwrite(g,['origImage' int2str(h), '.jpg']);
    
end
%%
for i=1400:2700
    g1=rgb2gray(read(v,i));
    g1=g1(ymin-1:ymax+1,xmin-1:xmax+1);
    imshowpair(binarizedArray(:,:,i),g1,'montage');
end
%%
